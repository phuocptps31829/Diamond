        ...
        <div className="flex flex-row items-center gap-3">
          ...
          <div className="flex flex-col overflow-hidden">
            <figcaption className="flex flex-row items-center whitespace-pre text-lg font-medium dark:text-white">
              <span className={`text-sm sm:text-lg ${isRead ?  "text-gray-500": "text-black" }`}>{title}</span>
              <span className={`mx-1 ${isRead ? "text-gray-500" : "text-black"}`}>·</span>
              <span className="text-xs text-gray-500">
                {formatRelativeDate(createdAt)}
              </span>
            </figcaption>
            <p className={`text-[12px] font-normal dark:text-white/60 ${isRead ? 'text-gray-500': 'text-black'}`}>
              {description}
            </p>
          </div>
        </div>
