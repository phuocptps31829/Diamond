...
export default function MiddleCharts({ dataAppointmentsByDoctor }) {
  const [appointmentByAge, setAppointmentByAge] = useState({
    male: 0,
    female: 0,
  });
  const [yearNow, setYearNow] = useState(new Date().getFullYear());
  const [dataTotalAppointmentsByYear, setDataTotalAppointmentsByYear] = useState([]);
  ...
  useEffect(() => {
    ...
    const filteredDataLastYear = dataAppointmentsByDoctor.data.filter((item) => {
      const itemYear = new Date(item.time).getFullYear();
  ...
  }, [yearNow, dataAppointmentsByDoctor]);
      ...
      <div className="mt-6 flex-1 rounded-md bg-white px-6 py-4 shadow-sm">
        <div className="mb-2 flex items-center justify-between">
          ...
          <div className="flex flex-col items-center">
            <div className="text-[17px] font-bold">{dataTotalAppointmentsByYear.length} lịch hẹn trong năm {yearNow}</div>
            <div className="mt-1 text-[14px] text-[#B8B8B8] flex gap-1"> 
              <span className={`${ growthRate >= 0 ? "text-[#00E396]" : "text-red-500" } flex gap-1 items-center`}>
                {growthRate >= 0 ? <FaSortAmountUp /> : <FaSortAmountDown />} {Math.abs(Math.round(growthRate))} %
              </span> 
              so với năm trước
            </div>
          </div>
        ...
        <div className="h-[230px] w-full">
          <LineChart dataTotalAppointmentsByYear={dataTotalAppointmentsByYear} />
        </div>
      </div>
      <div className="w-full flex-1 gap-6 rounded-md bg-white p-4 pt-2 shadow-sm mt-6">
