...
const ActionMenu = ({ row }) => {
  ...
  const { mutate: deleteMedicineCategoriesMutation, isPending } = useMutation({
    mutationFn: (id) => medicineApi.deleteMedicineCategories(id),
    onSuccess: () => {
            ...
            <DropdownMenuItem className="flex w-full items-center gap-2">
              <AlertDialog>
                ...
                <AlertDialogContent>
                  <AlertDialogHeader>
                    <AlertDialogTitle>
                      Bạn có chắc chắn muốn xóa danh mục thuốc này?
                    </AlertDialogTitle>
                    <AlertDialogDescription>
                      Hành động này không thể hoàn tác. Danh mục thuốc sẽ bị xóa
                  ...
                  <AlertDialogFooter>
                    ...
                    <AlertDialogAction
                      onClick={ () =>
                        deleteMedicineCategoriesMutation(row.original._id)
                      }
                    >
                      Xóa
                    </AlertDialogAction>
