...
const NewsCard = ({ newsItem, className, firstNews }) => (
  <Link
    ...
    className={`flex h-full overflow-hidden rounded-md bg-[#ffffff84] backdrop-blur-lg ${className}`}
  >
    {firstNews ? (
      <>
        ...
        <div className="p-3">
          <h2 className="my-2 text-[14px] font-bold md:text-[20px]">
            {newsItem.title}
          </h2>
      ...
      <>
        ...
        <div className="w-full p-3 pt-2">
          <h2 className="mb-2 text-[14px] font-bold">{newsItem.title}</h2>
          <div className="line-clamp-2 overflow-hidden text-ellipsis text-[12px] text-[#6D7280]">
            {newsItem.shortDescription}
          </div>
