...
const Forget = () => {
  ...
  const { mutate: changePasswordForgotMutation, isPending: isChangePassword } = useMutation({
    mutationFn: (data) => {
  ...
  const { mutate: checkOtpForgotPasswordMutation, isPending: isOtpPending } = useMutation({
    mutationFn: (data) => {
      return authApi.checkOtpForgotPassword(data);
    },
    onSuccess: async (data) => {
      await AsyncStorage.setItem("otpForgot", data.data.OTP);
      await AsyncStorage.setItem("tokenForgotChangePassword", data.data.otpToken);
    ...
    >
      ...
      {verifyOtp ? (
          ...
          <View className="flex-row justify-between mt-3 ">
            ...
            <TouchableOpacity onPress={() => handleSendAgainOtp()} disabled={isPending || timeLeft > 0} className={`${isPending || timeLeft > 0 ? "opacity-50" : ""}`}>
              <Text className="font-semibold text-[12px]">
                {isPending ? <ActivityIndicator size="small" color="blue" /> : "Gửi lại mã OTP"}
              </Text>
            </TouchableOpacity>
          </View>
          <TouchableOpacity
            onPress={() => verifyOtpSubmit()}
            disabled={isOtpPending || isChangePassword}
            className="bg-[#209bdd] py-3 mt-7 rounded-md items-center"
          >
            <Text className="text-white uppercase text-sm font-bold">
              {isOtpPending || isChangePassword ? <ActivityIndicator size="small" color="white" /> : "Xác thực"}
            </Text>
          </TouchableOpacity>
          <View className="flex flex-row items-center mt-5 justify-center space-x-1">
            ...
            <TouchableOpacity>
              <Text className="text-[14px] font-bold text-blue-500 underline"  onPress={() => router.back()}>
                Đăng nhập
              </Text>
            </TouchableOpacity>
          </View>
        ...
        <View className="flex-1 px-6 pt-8 bg-white">
          <Input control={control} name="phoneNumber"  error={errors.phoneNumber?.message} placeholder="Nhập số điện thoại" isInputPw={false} />
          <TouchableOpacity city className="bg-[#209bdd] py-3 mt-4 rounded-md items-center"
