...
const TabIcon = ({ image, color, name, focused }) => {
  return (
    <View
      className={ `${name === "Trang chủ" ? "absulute -top-5 gap-2" : "gap-1"
        } flex justify-center items-center pt-5 ` }
    >
      <View
        className={ `${name === "Trang chủ" ? "p-2 rounded-full bg-[#007BBB]" : ""
          } ` }
      >
        <Image
          source={ { uri: image } }
      ...
      <Text
        className={ `${name === "Trang chủ" ? "font-bold text-[12px]" : "text-[11px]"
          } ` }
        style={ { color: color } }
      >
        { name }
